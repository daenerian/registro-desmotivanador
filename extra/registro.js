...
const conexion = indexedDB.open('registroUsuarios', 1);
...
conexion.onupgradeneeded = (e) =>{
	db = e.target.result;
	const obAlmacenados = db.createObjectStore('usuarios', {
		autoIncrement: true
	});
}
...
function mostrarFormulario(){
	...
	const mensaje = document.getElementsByTagName('p')[1];
...
function comprobarEdad(evento){
	...
	if(diferencia<10 && !formularioProvincia.classList.contains('escondido')){
		...
		cambiarTamano(1530, 1162);
		return;
	}
	
	perry.classList.remove('escondido');
	cambiarTamano(1440, 1092);
...
function registrarUsuario(){
	...
	main.style.height = '393px';
	
	// Recoger datos
	const email = document.getElementsByTagName('input')[0];
	const contrasena = document.getElementsByTagName('input')[1];
	const fechaNac = document.getElementsByTagName('input')[2];
	const comunidad = document.getElementsByTagName('select')[0];
	const provincia = document.getElementsByTagName('select')[1];
	...
	const datos = {
		...
		comunidadAutonoma: comunidad.value,
...
/**
* Función que agrega un objeto a la base de datos del navegador
*/
function altaFila(datos){
	const transaccion = db.transaction(['usuarios'], 'readwrite');
	const objetos = transaccion.objectStore('usuarios')
	const conexion2 = objetos.add(datos);
	console.log('Alta exitosa');
}

/**
* Función que consulta los datos de la base de datos del navegador
*/
function consultarFilas(){
	const transaccion = db.transaction(['usuarios'], 'readonly');
	const objetos = transaccion.objectStore('usuarios');
	const conexion2 = objetos.openCursor();
	let datos = [];
	
	conexion2.onsuccess = (e) => {
		...
		if(cursor){
			...
			const imagen = document.getElementsByClassName('imagen')[0];
			let p = imagen.getElementsByTagName('p')[0];
			
			if(p!=undefined){
				imagen.removeChild(p);
			}

			p = document.createElement('p');
			p.classList.add('titulo');
			imagen.appendChild(p);
			p.appendChild(document.createTextNode('Usuarios registrados: ' + datos.length));
